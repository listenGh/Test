...
//                        window.console.log($("#" + id).find("input[name=item]:checked").val() + "??????????");
                        answer.push($("#" + id).find("input[name=item]:checked").val());
//                        console.log(answer + "??????");
                        //           alter("checkQues"+checkQues[i]);
                    }
                }
...
//                    console.log($("#" + id).find("input[name=item]:checked").val() + ">>>>>>>>");
                    answer.push($("#" + id).find("input[name=item]:checked").val());
//                    console.log(answer + ">>>>>>>");
            ...
            $(function () {
                ...
                $("#submitQuestions").click(function () {
                    ...
                    for (var i = 0; i < answer.length; i++) {
                        ...
                        if (questions[i].questionAnswer === answer[i]) {
                            score += 5;
                        }
                    }

                    alert("已做答:" + ($(".clickQue").length) + "道题,还有" + (questions.length - ($(".clickQue").length)) + "道题未完成," + "一共获得" + score + "分！" + "一共答对了" + (score / 5) + "道题目！");
                    //信息传入数据库

                    $.ajax(
                            {
                                url: "/Test/getScore", //访问路径
                                type: "POST", //访问方式
                                //传入服务端的数据
                                data: JSON.stringify(score+"!!!"+sId ),
                                //                        data:JSON.stringify(),
                                dataType: "json",
                ...
                window.location.replace("http://localhost:8080/Test/OperatingPageOfStudent.jsp");
                })
                //进入下一题
    ...
    <body>
        ...
        <div>
            <input type="hidden" value="<%=request.getAttribute("sId")%>" class="sId"/>
